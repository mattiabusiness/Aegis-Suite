...
export async function getCurrentUserBusiness(
  ...
  try {
    ...
    const { data: business } = await supabase
      .from('businesses')
      .select('id, slug, name, vertical, email, phone, website, address_street, address_city, address_province, address_postal_code, address_country, latitude, longitude, logo_url, cover_image_url, primary_color, secondary_color, timezone, currency, booking_advance_min, booking_advance_max, cancellation_policy_hours, auto_confirm_bookings, description, short_description, owner_id, is_active, is_verified, created_at, updated_at, business_type, onboarding_completed, onboarding_step, workstations, roi_data, terms_accepted_at, articles_1341_accepted_at, is_public, shampoo_price')
